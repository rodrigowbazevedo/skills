...
description: Saves a completed plan as a GitHub issue (create new, update existing, add a sub-issue, or append a comment) and auto-links it into any open PR on the branch (appends `Closes #<n>`). Invoke at two moments. (1) Immediately after every ExitPlanMode call — but DO NOT block asking whether to create the issue; instead record a deferred marker and let implementation start right away. (2) Before the first commit or PR on the branch, when a deferred plan-issue marker is pending — that's when you ask how to save the plan. Triggers: right after ExitPlanMode; before committing/opening a PR with a pending plan issue; or when the user says "create the issue now" / "save the plan as an issue".
---
...
Decide which situation you're in — this selects the option set in B3.

1. **Source issue `N` present** → classify it by label: `gh issue view N --json labels --jq '.labels[].name'`.
   - Has the `plan` label → **revising a plan issue** (`N` is itself a saved plan).
   - No `plan` label → **started from a real issue** (`N` is a feature/bug/task you're implementing).
...
| **Revising plan issue #N** | Update #N body · Sub-issue under #N · Comment on #N · Discard |
| **Started from real issue #N** | Sub-issue under #N · Comment on #N · Discard (#N already tracks it) · Standalone new issue |
...
- **Create new issue** / **Update #N body** — standard create or body edit; record the issue number for PR linking.
...
- **Comment on #C** — append the plan as a comment; then **ask** whether the PR should also `Closes #C`.
...
|------|---------|--------------|
| `references/dispositions.md` | Bash for each Phase B disposition (create, update, sub-issue, comment, discard) + the hardened PR-linking (`Closes #N`) routine | In Phase B once the user has chosen a disposition. Phase-A-only invocations never need it. |
